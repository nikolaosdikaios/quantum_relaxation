...
axa.set_title("(a)  five fields: global two-parameter fit (dashed)\n"
              "vs per-field Bloch (dotted)", fontsize=13.5, pad=8)
...
axins.plot(tgrid, data[w_lo] - pair_z(D_hat, tau_hat, w_lo, tgrid, 6),
           color="#B5502A", lw=1.4, label="transition-current")
...
x = np.arange(len(fields)); wdt = 0.38
...
axb.bar(x - wdt / 2, rmsB_f, wdt, color="#2E5F8A",
        label="Bloch, one $T_1$ per field")
axb.bar(x + wdt / 2, rmsK_f, wdt, color="#B5502A",
        label=r"transition-current, global $(\Delta\omega,\tau_c)$")
...
axb.set_title(rf"(b)  residuals: $\Delta$AIC $= {aic_B - aic_K:+.0f}$"
              "\n(transition-current model has the lower AIC)", fontsize=13.5, pad=8)
axb.legend(fontsize=11.5, frameon=False)
