...
Checks, in manuscript order:
  ...
  8  Kubo lineshape short-time expansion agrees with the pair to O(t^2)
  9  Onsager-Casimir parity identity  E G E = G^T
 10  Gibbs/Witten vacuum is an exact stationary state
...
ok("Markov/exact initial-decay ratio = kappa^2 at t ~ 1/D",
...
# vacuum: Q e^{-f} = 0 with e^{-f} the Gibbs half-density
f1, f2 = sp.symbols('f_1 f_2')
Qdb = sp.Matrix([[-sp.sqrt(sp.exp(f2 - f1)), sp.sqrt(sp.exp(f1 - f2))]])
vac = sp.Matrix([sp.exp(-f1 / 2) * sp.exp(f1 / 2), sp.exp(-f2 / 2)
                 * sp.exp(f2 / 2)])
ok("Q e^{-f} = 0: equilibrium carries no current",
   sp.simplify((Qdb * sp.Matrix([1, 1]))[0, 0]
               - (sp.sqrt(sp.exp(f1 - f2)) - sp.sqrt(sp.exp(f2 - f1)))) == 0)
