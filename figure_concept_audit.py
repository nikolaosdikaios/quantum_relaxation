...
ok(f"Markov reaches half amplitude {ratio:.1f}x sooner (text says 8.8)",
   8.0 < ratio < 9.5)
ok("pair oscillates in the rigid regime (physical: it is underdamped)",
   (pair < -0.05).any() and D * tc > 0.5)

# ---------------- Fig. 2(b): positivity map --------------------------------
print("\nFig. 2(b)  positivity domain")
...
ok("small-kappa cell is non-negative (blue, certified)", lo_k > -1e-9)
...
ok("present formulation launches with zero slope (quadratic)",
...
def audit_topology():
    print("\nFig. topology  state complex and incidence matrix")
    verts = ["aa", "ab", "ba", "bb"]
